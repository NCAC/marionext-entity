...
import { ElementType } from "@ncac/marionext-types";

type TypesOrModelsOrCollections = Array<
  IType | (new (...args: any) => Model<any, any>)
>;
...
export interface IOrType<T extends IType> extends IType {
  <TOrTypes extends TypesOrModelsOrCollections>(
    params: IOrTypeParams & { or: TOrTypes }
...
export class OrType extends Type {
  ...
  constructor({ or, ...otherParams }: IOrTypeParams) {
    super(otherParams);
  ...
  prepare(originalValue: any, key: string, model: any) {
  ...
  equal(selfValue: any, otherValue: any, stack: any) {
